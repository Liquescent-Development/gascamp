...
- **an explicit backpressure policy — and the cap is a STOP, not a kill** (AMENDED by cp-1; operator-approved. The original text read *"a subscriber whose buffer crosses the cap is dropped"*, and that is what the implementation deliberately does NOT do):
  ...
  - ***Known residuals, accepted for cp-1 and inherited by cp-2 — BOTH directions of the same imprecision:***
    - ~~**Too lenient:** a peer accepting a healthy chunk per interval clears the stall timer indefinitely and can hold a buffer and one of `MAX_SUBSCRIBERS` slots forever.~~ **RESOLVED (#121)** by the **time-at-cap bound** below. The residual was real: `camp watch` is precisely the thing an operator leaves open in a scrolled-back terminal all day.
    - **Too strict:** the implementation clears the stall timer only when campd's own `write()` **accepts bytes**, and a non-blocking write on a unix socket returns `EAGAIN` with **zero** bytes until free space reaches the kernel's **low-water mark** (~2 KiB on macOS). So the code enforces *"a peer whose socket has not freed a low-water mark's worth"*, not literally *"a peer that stopped reading"*: a peer sipping less than ~4 KiB per `SUBSCRIBER_STALL_TIMEOUT` is dropped **while still reading**. At the shipped 30 s default that rate is indistinguishable from a stopped peer, so the reach is small — but the spec's words and the code's behaviour differ, and that is written down here rather than discovered later.
    - A byte-rate floor (which would fix the first) or a read-progress signal independent of the write path (which would fix the second) is the honest resolution. Both were recorded as **cp-2 obligations**, not silently deferred. **The first is now resolved (#121); the second remains open** — it needs a read-progress signal campd does not have, and no shipped code claims otherwise.
- **a second drop rule: a bound on TIME AT CAP WITHOUT HEADWAY** (added by #121, resolving the "too lenient" residual above). `SUBSCRIBER_STALL_TIMEOUT` alone asks only *"has this peer stopped reading?"*, and **one accepted byte answers yes** — so a peer sipping just enough to keep campd's writes flowing, but never enough to work off its buffer, is invisible to it while holding a slot and a full cap of memory indefinitely. A subscriber is therefore **also** dropped when it has stayed at the cap for **`AT_CAP_STALL_INTERVALS` (10) stall intervals — 300 s at the default — WITHOUT DRAINING A CAP'S WORTH**. The clock starts when the buffer first reaches the cap and **resets each time the peer has accepted a whole cap of bytes since it started**; a peer that keeps moving a cap per window is never dropped. The drop is the **same** `subscriber.dropped` event, with the same fields: a drop for a new reason is not a new event.
  - ***Why a headway bound and not a byte-rate floor.*** It bounds the **actual harm** — a held slot and a held buffer the peer is not working off — instead of a proxy for it; it needs **one timestamp, one byte counter, and no new throughput tunable**; and the threshold (one cap per `AT_CAP_STALL_INTERVALS` windows) is **derived** from the cap and stall interval that already exist, so there is exactly one stall concept to configure (`CAMP_SUBSCRIBER_STALL_TIMEOUT_MS` scales both). A raw floor would need a defensible bytes-per-interval number, and every number is wrong in **both** directions: too low and the trickle reader survives it, too high and a legitimately slow consumer **on a quiet stream** is killed for the stream being quiet — and a headway bound self-scales, since on a quiet stream `out` never reaches the cap and the clock never starts.
  - ***Why headway and not "drained to empty."*** `out` refills to the cap after every partial write, so it reaches empty only when the whole **backlog** is gone. Keying the reset on empty would drop a peer for **joining far behind** — the buffer-SIZE kill this amendment forbids — because during catch-up a file read (256 KiB/wake) always outruns a socket, so `out` is pinned at the cap the entire time a large history streams, **however fast the peer reads**. Keying on a cap of headway instead, a client that joins far behind the tail is **never** dropped for how far behind it is — only a peer that cannot move even one cap in 300 s (~3.5 KiB/s at the default) is, and that is a peer not keeping up with a trickle.
  - ***Why 10.*** It must be **> 1**, or the window is one stall interval and a peer merely BEHIND is dropped on the same deadline as one that is DEAD — which this amendment forbids in as many words. An order of magnitude is the smallest step that is plainly not the zero-accept rule re-litigated, and it puts the survival floor (~3.5 KiB/s) far below any real terminal, pipe, or socket consumer while staying far above zero (the zero-accept rule's own standard).
  - ***Idle is still free (invariant 1).*** The rule **arms no timer of its own**. Any subscriber whose at-cap clock is running has a non-empty buffer, and the pump loop only comes to rest on `WouldBlock`, which stamps the zero-accept clock — so the existing stall deadline is already armed for it, and re-armed every time the peer accepts a byte. The at-cap drop fires on the next wake the peer's own trickle causes, or on that deadline at the latest.
