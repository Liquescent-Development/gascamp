...
- **a second drop rule: a bound on TIME AT CAP** (added by #121, resolving the "too lenient" residual above). `SUBSCRIBER_STALL_TIMEOUT` alone asks only *"has this peer stopped reading?"*, and **one accepted byte answers yes** — so a peer sipping a socket buffer's worth per interval is invisible to it while holding a slot and a full cap of memory indefinitely. A subscriber is therefore **also** dropped when its buffer has been **continuously at the cap for `AT_CAP_STALL_INTERVALS` (10) stall intervals** — **300 s** at the shipped default. The drop is the **same** `subscriber.dropped` event, with the same fields: a drop for a new reason is not a new event.
  - ***Why time-at-cap and not a byte-rate floor.*** It bounds the **actual harm** — a held slot and a held buffer — instead of a proxy for it; it needs **one timestamp and no new throughput tunable**; and it derives from the **existing** stall interval, so there is exactly one stall concept to configure (`CAMP_SUBSCRIBER_STALL_TIMEOUT_MS` scales both). A floor would need a defensible bytes-per-interval number, and every number is wrong in **both** directions: too low and the trickle reader survives it, too high and a legitimately slow consumer **on a quiet stream** is killed for the stream being quiet.
  - ***Why 10.*** It must be **> 1**, or the rule is indistinguishable from the zero-accept rule and a peer that is merely READING dies on the same deadline as one that is DEAD — which this amendment forbids in as many words. An order of magnitude is the smallest step that is plainly not that rule re-litigated, and it is generous against the one healthy state that **is** continuously at the cap: a catch-up (a file read at 256 KiB/wake always outruns an ~8 KiB socket).
  - ***The bound it imposes, stated.*** `out` is refilled to the cap after every partial write, so it reaches EMPTY only when the **source** runs dry. The window therefore bounds the time to consume the whole **backlog**, not one cap's worth of it: at 300 s a 50 MB history needs ~170 KB/s sustained, against a local unix socket that does orders of magnitude more. **This is not the buffer-SIZE verdict this amendment forbids** — a client that joins far behind the tail is never dropped for how far behind it *is*, only for failing to move that backlog **at all** for 300 s.
