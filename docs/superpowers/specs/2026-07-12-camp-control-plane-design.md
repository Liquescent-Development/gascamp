...
- **an explicit backpressure policy — and the cap is a STOP, not a kill** (AMENDED by cp-1; operator-approved. The original text read *"a subscriber whose buffer crosses the cap is dropped"*, and that is what the implementation deliberately does NOT do):
  ...
  - ***Known residuals, accepted for cp-1 and inherited by cp-2 — BOTH directions of the same imprecision:***
    - **Too lenient:** a peer accepting a healthy chunk per interval clears the stall timer indefinitely and can hold a buffer and one of `MAX_SUBSCRIBERS` slots forever. It **is** reading, so it is not stalled — and any byte-rate floor is a policy number nobody has evidence for. `camp watch` is precisely the thing an operator leaves open in a scrolled-back terminal all day.
    ...
    - A byte-rate floor (which would fix the first) or a read-progress signal independent of the write path (which would fix the second) is the honest resolution. Both are recorded as **cp-2 obligations**, not silently deferred.
