...
- **an explicit backpressure policy: a subscriber whose buffer crosses the cap is dropped, loudly, with an event** (`subscriber.dropped`, naming the session and the high-water mark). It is never blocked on, and events are never silently discarded. campd's loop is single-threaded — a stalled subscriber that campd waits on is a daemon-wide wedge (the bug class of issue #55);
...
- the kill is evented with the reason **`"adoption: unanswerable permission request"`** — a named reason, greppable, never a generic crash;
...
- **Subscriber backpressure:** a subscriber that stops reading is dropped at `subscriber_buffer_bytes` with the `subscriber.dropped` event; campd never blocks. The hello arrives within `REQUEST_TIMEOUT` even against a busy daemon.
