...
> **Plan-gate APPROVED 2026-07-15** (adversarial 4-panel: contract/interface/execution/critic, fresh panelists each round). R1 = 3 APPROVE / 1 REJECT (critic: B1 — permission request_id undiscoverable through the overseer's socket surface, skill text false + exit test hardcoded it; B2 — stale "answer lands when cp-3 ships" text). R2 revision (3754d92) combined B1+B2 into Task 3B (render request_id on attach's BLOCKED line from the control_request frame's top-level id, NO wire change; skill names attach/subscribe as the id source; Task 5 rewritten to DISCOVER-not-hardcode, coupled to 3B) → R2 gate 3 APPROVE / 1 REJECT (critic: one residual stale surface, the attach.rs module header). R3 fix (cdf1988) added the module header to Task 3B's edit list → critic re-confirm APPROVE. UNANIMOUS. Non-blocking notes carried: (1) Arm-B chmod-000 sufficiency test carries a stated non-root assumption + a runtime root self-check that honestly SKIPS (never a vacuous green) — but Rust marks a root-local early-return as plain "ok"; CI (the merge gate) is non-root so it holds. (2) The `SessionInfo` pid-absence assertion is an explicit future-regression tripwire, not run coverage. (3) Task 3B renders the id but deliberately does NOT wire attach's interactive /allow-//deny loop — that stays cp-4's deferral.
...
**Why this task exists (round-1 gate BLOCKER 1 + 2).** The overseer's answer path is `camp decide <session> <request_id>`, but the `request_id` is discoverable through the socket ONLY on the `session.subscribe` stream: the `can_use_tool` frame carries a top-level `request_id` (verified: `tests/fake-agent.sh` emits `{"type":"control_request","request_id":"…","request":{"subtype":"can_use_tool",…}}`; `attach.rs`'s own unit test feeds `"request_id":"r1"`). But `attach.rs`'s `render_event` `can_use_tool` arm currently DROPS that id and renders `"(answer lands when cp-3 ships)"` — a falsehood now that cp-3 is merged. The same falsehood appears in TWO other surfaces of the same file: the module `//!` header ("answering a permission request drops in when cp-3's verb lands") and the `enum Action` doc comment ("the permission-answer actions … drop in here when cp-3's … verb lands"). All three must move together, or the file contradicts itself (a render line saying "answer: camp decide" under a header saying the answer awaits cp-3). `SessionInfo` has no `request_id` field and the wire is frozen (this plan's Global-Constraints rule), so the id must be surfaced where it already lives — in the rendered stream line. This task renders the id and corrects all three stale texts to name the real answer path, WITHOUT wiring attach's interactive `/allow`//deny loop (that remains cp-4's separate deferral — scope boundary drawn by the gate).

**Files:**
- Modify: `crates/camp/src/cmd/attach.rs` — three stale surfaces, all corrected in one pass: (1) the module-level `//!` header (its last sentence, "answering a permission request drops in when cp-3's verb lands"); (2) the `render_event` `can_use_tool` arm (the `!! BLOCKED …` line); (3) the doc comment on the `enum Action`.
...
- [ ] **Step 4a: Correct the stale module header (`//!` block, its last sentence)**

The module header (the `//!` doc block, `crates/camp/src/cmd/attach.rs`, its final sentence "From here you send a turn or interrupt; answering a permission request drops in when cp-3's verb lands.") predates cp-3's merge and now contradicts the render line Step 3 adds three lines below it. Reword only that last sentence to match the Step 4 `Action`-enum wording — out-of-band answering via `camp decide` is available NOW; only the interactive in-attach action remains cp-4's deferral. Replace it with:

```rust
//! From here you send a turn or interrupt. Answering a permission is
...
4. **`camp attach` text touched (Task 3B) — a correction, not new machinery.** cp-4 shipped `camp attach` asserting `"answer lands when cp-3 ships"` in THREE surfaces of one file — the module `//!` header, the `render_event` BLOCKED line (which also dropped the permission `request_id`), and the `enum Action` comment. cp-3 IS merged, and cp-5's contract makes the operator a full §5.4 client including answer-a-permission — so leaving those intact would ship a live surface telling the human the answer path does not exist AND would hide the only socket source of the id the overseer needs. Task 3B renders the id (already present on the stream frame — no wire change) and corrects all three texts to name the real `camp decide` path. It deliberately does NOT wire attach's interactive `/allow`//deny loop: that remains cp-4's separate deferral (scope boundary set by the gate). Corrective + additive; removes a false claim, adds no protocol.
