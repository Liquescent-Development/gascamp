...
> **Plan APPROVED by the fresh Opus review pass, 2026-07-07 (relayed by the lead), at tip b2001cf.** Both blocker fixes verified genuinely fixed (the disposition split checked against vocab.rs:42 and the gc fixture; the per-invocation budget verified against the real event_loop.rs — `orders::settle` has exactly one call site inside `event_loop::settle`, which every route funnels through). Implementation notes carried from the approval: (1) the budget granularity is per-`event_loop::settle`-INVOCATION — one poll iteration can invoke settle multiple times (SIGCHLD + poke + tail), each independently converging and budget-bounded; code and comments say "per settle invocation", never "per wake"; (2) the `RUNS_SUBDIR` const also replaces the test-code `"runs"` literal at orders/mod.rs:764.
>
> **First-pass history:** Opus review 2026-07-07: REJECT on two plan-internal blockers, design otherwise sound; decisions 2, 4, 5, 7, and 9 ACCEPTED as argued. This revision resolves Blocker A (disposition-vocabulary contradiction: `bead.closed` never carries a `"pass"` disposition — the run-level disposition lives solely in `run.finalized`; anchor exhaustion closes keep `hard_fail|soft_fail`, exactly the `⊆ gc.on_exhausted` pin), resolves Blocker B (the #17 fire budget is per-WAKE — reset once at `event_loop::settle` entry — with a through-converge regeneration test), and folds in the review's five non-blocking notes. Resubmitted for a fresh pass.
...
- Siblings in flight: phase-11-patrol-adoption (shares daemon/event-loop area) and phase-14-export-bridge. On a sibling merge notice: rebase onto current main, re-run all gates before continuing. Merge order with phase-11 is genuinely open (both in plan revision) — whoever is clean first merges first; the second rebases.
- **Phase-11 coordination pins (lead-relayed, 2026-07-07 — phase-11's Decision C accepted):**
  1. Worker stdout captures (`sessions/<name>.json`) become stream-JSONL when Phase 11 merges. **This plan never parses the capture**: check verdicts come from check-script exit codes; structured step output comes ONLY from the close event's `data.output` (`camp close --output-json`). Implementers: do not reach for the capture files; if that ever changes, the parse rule is "the line whose `type == "result"`".
  2. `orders::settle` and the event-loop threading grow one patrol parameter under Phase 11 — expect signature-line rebases in event_loop.rs/orders.rs for whichever phase merges second (Task 12's rebase gate).
  3. Phase 8's `Worker` struct gains fields and `launch()` writes a first stdin message under Phase 11. **This plan constructs no Workers**: attempt-bead respawns are `bead.created` events that flow through the unchanged `Dispatcher::converge` → `dispatch_one` → `launch` path, so that seam moving does not touch Phase 9 code.
  4. Phase-11 reserves `Token(3)` for its patrol watch pipe; connections move to 4+. **This plan adds no poll sources and no tokens** — check deadlines contribute to the poll timeout only, and check children are reaped via the existing SIGCHLD pipe (Token 2). Any change to that is announced to the lead first.
...
Orphan run dirs (review note 4): a `kill -9` between cook's run-dir write and its `append_batch` commit leaves `runs/<id>/` with no ledger record; reconciliation and fire-dedupe then re-cook under a NEW id, so recovery is idempotent but the orphan dir lingers. This is the crash window cook.rs's header already records as the safe direction (files-first). This task builds no sweep: it adds a sentence to `GraphRuntime::reconcile`'s doc comment naming the window and pointing at cook.rs's note.

> **SUPERSEDED (2026-07-16, issue #124).** The deferred sweep is now BUILT, as `camp doctor --orphan-runs` (list, read-only) and `--sweep-orphan-runs` (remove). It stays out of `reconcile` permanently, which is the part of the decision above that survives: campd startup is the worst moment to sweep, because campd is about to cook and a run dir with no `run.cooked` is exactly what a live cook looks like. The operator verb refuses while campd is up and skips any dir touched within `ORPHAN_RUN_SWEEP_GRACE`. See `cmd/doctor.rs::run_orphan_runs`.
