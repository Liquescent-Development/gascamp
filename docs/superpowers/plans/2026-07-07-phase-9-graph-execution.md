...
**Termination argument (issue #17 adjacency):** every graph append is guarded by a state precondition that the append itself destroys (claim requires `open`; attempt creation requires budget remaining; anchor close requires anchor not closed; finalization requires root open). So the settle fixpoint does bounded work per wake, and the new machinery cannot widen #17's class. The #17 class itself (self-triggering event orders) gets a cheap per-settle fire budget — Decision 8.
...
Decisions 1–7 and 9–13 are design rulings this plan is built on; Decision 8 is the issue-#17 closure the kickoff asked to flag. All are submitted for the plan review; 2, 4, 5, 7, 8, 9 most need an explicit ruling.
...
3. **Finalization = quiescence + mechanical aggregation.** A run finalizes when every anchor is closed or unsatisfiable-open, and none is in flight (`in_progress`, or open-and-satisfiable). Then, in ONE cursor transaction: skipped-closes for unsatisfiable anchors → root `bead.closed` → `run.finalized` (with `cause_seq` = the close event that triggered quiescence — the spec §13.3 cause chain). Aggregation table (fully mechanical):
   | run contains | root outcome | final_disposition |
   |---|---|---|
   | any anchor fail with `hard_fail` disposition (or plain fail) | fail | hard_fail |
   | else any skipped anchor | fail | soft_fail |
   | else any soft-failed anchor | pass | soft_fail |
   | else (all pass) | pass | pass |
...
8. **Issue #17 cheap closure: per-(order, settle) fire budget.** `FIRE_BUDGET: usize = 256` fires per order per settle call; beyond it the processor appends `order.failed {order, fired_seq: <matching event's seq>, error: "event-trigger fire budget (256) exhausted in one settle — likely a self-triggering order"}` instead of `order.fired`, and stops firing that order for the rest of the settle. Legitimate bursts are bounded by the pre-existing backlog; only regenerative loops (fire → cook → matching event → fire) hit the budget. Compile-time rejection was considered and rejected: `bead.closed` — the flagship trigger — is itself campd-produced. **Needs a ruling** (budget value, and the `fired_seq`-carries-the-cause-seq shape for the budget failure event).
...
11. **event_loop.rs seam touches (announced to the lead with this plan):** (a) thread a `&mut GraphRuntime` through `run`/`settle`/`serve_connection`/`drain_lines`/`reap_and_refill` signatures; (b) `graph.execute` call between `orders::settle` and `dispatcher.converge` in `settle`; (c) poll timeout becomes `min(runtime.poll_timeout(now), graph.poll_timeout(now))` — the same seam Phase 10 used; (d) one `graph.kill_expired(now)` call at wake head (after the fires computation) so a deadline-only wake enforces check timeouts; (e) `graph.reap_checks(ledger)` in `reap_and_refill` before the settle. No token changes, no restructuring, poke-arm untouched.
...
**Interfaces produced:** `EventType::{CheckPassed, CheckFailed, RunFinalized}` (`"check.passed"`, `"check.failed"`, `"run.finalized"`); `CAMP_OUTCOMES = ["pass","fail","skipped"]`; `CAMP_FAILURE_CLASSES = ["transient"]`; `bead.closed` data accepts optional `failure_class`, `final_disposition`, `output`.
...
```
  ...
  In `bead_closed`, after the outcome check: `failure_class` must be in `CAMP_FAILURE_CLASSES` and requires `outcome == "fail"`; `final_disposition` must be in `CAMP_FINAL_DISPOSITIONS`. `output` is any JSON (audit content; readers pull it from the event row). New arms in `apply`, each log-only with `deny_unknown_fields` structs:
...
```
  ...
  Validation: `check.passed`/`check.failed` require non-empty `run_id`/`step_id`, `attempt >= 1`, and event.bead set + known (the attempt bead); `check.failed` requires at least one of `exit_code`/`signal`/`timed_out:true`/`error`; `run.finalized` requires event.bead = `root`, `outcome ∈ CAMP_OUTCOMES`, `final_disposition ∈ CAMP_FINAL_DISPOSITIONS ∪ {"pass"}` (add `"pass"` by checking against `["pass","hard_fail","soft_fail"]` — introduce `pub const CAMP_RUN_DISPOSITIONS: &[&str] = &["pass","hard_fail","soft_fail"];` in vocab.rs, all present in gc's `final_disposition` pin).
...
**Interfaces produced:** `camp close <bead> --outcome pass|fail [--reason r] [--transient] [--output-json <file|->]`; `camp sling --formula <name> [--rig r]` (prints `"<run_id> root <root-bead>"`); `CampDir::runs_path() -> PathBuf` (`root.join("runs")` — the same path `execute_fire` already uses).
...
3. `BeadClosed`:
   ...
   e. bead belongs to a run (`run_id` set) → run `runtime::finalization(conn, ctx)`; on `Finalize{..}` append, in this same transaction: `bead.closed {outcome:"skipped", reason:"needs cannot be satisfied"}` for each open skipped anchor, then root `bead.closed {outcome, final_disposition}`, then `run.finalized {run_id, root, outcome, final_disposition, cause_seq: event.seq, soft_failed, skipped}`.
   Context loads for (3) come from the cache, loading on miss; a run dir that fails to load appends nothing and records `None` (evented once via `dispatch.failed`-style? No bead fits) — **ruling applied:** a missing/corrupt run dir closes the ROOT `{outcome:"fail", final_disposition:"hard_fail", reason:"run dir unreadable: <err>"}` + `run.finalized`, if the root is still open; that is the honest mechanical dead-end (nothing else can ever advance the run).
...
#[test] fn finalization_closes_root_with_cause_and_skips_unreachable() { /* two-step run,
    step2 needs step1; close step1 fail -> step2 closed skipped, root closed fail/hard_fail,
    run.finalized cause_seq == the step1 close seq */ }
...
- `poll_timeout(now)`: earliest child deadline − now, `Duration::ZERO` floor, `None` when no deadlines; event_loop combines: `let timeout = min_opt(runtime.poll_timeout(ts_now), graph.poll_timeout(Instant::now()));` (a small `fn min_opt(a: Option<Duration>, b: Option<Duration>) -> Option<Duration>` helper in event_loop.rs).
...
### Task 9: Issue #17 fire budget

**Files:**
- Modify: `crates/camp/src/daemon/orders.rs`

`OrdersRuntime` gains `fires_this_settle: HashMap<String, usize>` cleared at `settle` entry; the event-trigger loop in `CampdProcessor::process` increments per order and, at `FIRE_BUDGET` (const 256), appends `order.failed {order, fired_seq: event.seq, error: "event-trigger fire budget (256) exhausted in one settle — likely a self-triggering order"}` once and suppresses that order until the next settle. Module docs reference issue #17.

- [ ] **Step 1: Failing test:** an order on `event:bead.created` whose formula's cook creates beads (any formula does — the cook's own `bead.created` events re-match) must NOT hang `settle`: assert `settle` returns, exactly one budget `order.failed` for the order, and the event count is bounded (< budget × steps + slack). Use the existing `fixture`/`settle_all` helpers.
...
- [ ] **Step 3:** implement. **Step 4:** PASS + full `cargo test -p camp`. **Step 5:** Commit `fix(daemon): per-settle event-order fire budget — closes #17's unbounded settle class`.
...
- [ ] **Test 4 — transient retry exhaustion, hard vs soft table:** `retry max_attempts = 2`. (a) `on_exhausted = "hard_fail"`, plan `fail-transient\nfail-transient`: anchor fail/hard_fail, root fail/hard_fail. (b) `on_exhausted = "soft_fail"` in a two-step formula whose second step is independent and passes: anchor fail/soft_fail, root **pass**/soft_fail, `run.finalized.soft_failed == ["fetch"]`. (c) soft_fail with a dependent step: dependent closed `skipped`, root fail/soft_fail. Refold clean after each.
