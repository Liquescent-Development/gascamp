...
> **CLOSED (#121), and not by cp-2 — cp-2 neither implemented this obligation nor re-deferred it, and a deferral-chain audit found it still open.** The resolution is a **bound on time-at-cap WITHOUT HEADWAY**, not the byte-rate floor: `OutBuf::at_cap_since` is stamped when the cap first stops a source and reset each time the peer has drained a whole cap's worth since (`drained_at_cap`); a subscriber that stays at the cap for `AT_CAP_STALL_INTERVALS` (10) stall intervals **without moving a cap of headway** is dropped with the **same** `subscriber.dropped` event as a zero-accept peer. (An earlier revision keyed the reset on the buffer draining to EMPTY — a review (#121) caught that as a **buffer-SIZE kill in disguise**: `out` refills to the cap after every write and only empties when the whole backlog is gone, so it would have dropped a healthy catch-up reader for merely joining far behind. The headway reset fixes that: a peer moving a cap per window is never dropped at any backlog size.) See the §4.4 amendment in `docs/superpowers/specs/2026-07-12-camp-control-plane-design.md` for the full rationale (why headway not empty; why not a floor; why 10; why it arms no timer).
...
11. **`a_peer_that_accepts_nothing_is_dropped_at_the_stall_timeout`** (**R1's drop rule**) — a client that never reads (`CAMP_SUBSCRIBER_STALL_TIMEOUT_MS=200`): assert `blocked_since` is stamped on the zero-accept write, that `poll_timeout` arms **the stall deadline** (nothing else will ever fire for it), and that `Drop(subscriber.dropped)` fires with `buffered_bytes` = the high-water. **And the converse:** a client that accepts even **one byte** clears `blocked_since` and is never dropped. *(That converse remains exactly right for THIS rule, which asks only "has the peer stopped reading?" — but it is no longer the whole policy: the slot-holding residual it used to imply is now bounded by the time-at-cap rule, #121. The test is renamed `..._is_never_dropped_by_the_zero_accept_rule` to say which rule it pins, and the time-at-cap rule has its own tests: a trickle reader that never moves a cap per window IS dropped, and a RATE-LIMITED catch-up reader that keeps moving a cap per window — pinned at the cap over a backlog that never empties — is NOT, #121. That second test is the false-positive guard; under the earlier drain-to-empty reset it failed, which is how the buffer-SIZE-kill regression was caught.)*
