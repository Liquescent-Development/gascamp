...
- **In-flight `claude -p` workers are reconciled at the next start, and that is correct.** campd appends `campd.stopped`, unlinks, and exits without waiting on its worker children; `patrol::adopt` reconciles them at the next start (crash-only, spec §8.5). This is *exactly* what today's socket `Request::Stop` does — so the "identical outcome to `Request::Stop`" contract holds, and Phase 1 adds no new reconciliation burden. What happens to the workers themselves depends on **who** got the signal, and the two cases differ (PR #69 review, M5):
  - **SIGTERM** (a supervisor, `kill <campd-pid>`) reaches campd's pid alone: the workers keep running, are reparented to init, and adoption picks them back up.
  - **A real Ctrl-C** delivers SIGINT to the whole foreground process group, and a foreground `camp daemon`'s workers inherit that pgroup — so they take the signal too and die. Adoption then reconciles them as `session.crashed`, which is the same crash-only path. (An *auto-started* campd is immune either way: `autostart.rs:73` sets `.process_group(0)`.)
