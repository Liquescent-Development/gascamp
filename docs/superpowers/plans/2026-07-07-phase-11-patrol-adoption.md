...
> **Status: AWAITING PLAN APPROVAL** (master plan decision 10 as amended: automated Opus plan review relayed through the lead). Decisions flagged **[NEEDS RULING]** below are the load-bearing calls the review must accept or overrule; each carries a pre-decided fallback so a rejection changes scope, not structure.
...
- **Spec is authoritative:** `docs/design/2026-07-05-gas-camp-design.md`; §4 decisions settled. If implementation reality contradicts the spec, stop and update the spec via PR in the same change (spec edits are serialized through the lead — this plan proposes exactly one sentence, Task 11.16, flagged).
...
- Docs new: `docs/design/2026-07-07-phase-11-probe-findings.md` (probes P1–P3, verbatim evidence). Docs modified: spec §10 (ONE sentence, Task 11.16, **[NEEDS RULING]**).
...
**Decision C — campd-spawned workers move to stream-json input mode with campd-held stdin. [NEEDS RULING — the load-bearing decision; also flag to phase-9]**
...
**Interfaces:** none (docs + comments). The carried PR #14 review note (transcript-path fidelity) is resolved HERE.
...
```bash
git add docs/design/2026-07-07-phase-11-probe-findings.md crates/camp/src/daemon/spawn.rs
git commit -m "docs: phase 11 probe findings; munge fidelity verified per-char (resolves PR #14 note)"
```
...
```
Semantics:
- `observe` (inside the cursor transaction; memory-only, never I/O): `session.woke` with a `transcript_path` → queue Track (owned = Child if `event.actor == "campd"` else Annotate; threshold = agent `stall_after` override or camp default; the tracked state keeps bead, agent, rig, claude_session_id, transcript_path, and worktree from the payload — the nudge-resume and restart paths need them). `session.stopped`/`session.crashed` → queue Untrack. `bead.closed` for a tracked worker's bead → queue Release (Decision C2) + `ladder.forget(bead)`. Any event matching Decision J's three keys → timer reset + `ladder.on_activity(bead)`.
...
```
Flow (Decisions F + G, in order): (1) per live registry row: skip sessions already tracked as this campd's own children (they are alive by definition — the dispatcher holds them); probe the rest; dead → `session.crashed {name, reason:"adopt: process not found"}`; alive + bead closed/absent → release (for non-children: `/bin/kill` + verified + `session.stopped {name, reason:"released after bead close"}` appended directly — counted `released`); alive + bead open → track + arm fresh (`Owned::AdoptedPid`), counted `rearmed`. (2) sweep `<camp>/worktrees/*` per Decision G (`swept` = removed+reaped, `kept` = newly kept). `probe_alive` errors (pgrep/ps missing or failing to exec) are hard errors — fail fast, adopt reports it, campd startup surfaces it.
...
#[test] fn adopt_is_idempotent() { /* run twice; second AdoptSummary is all zeros; no duplicate events */ }
```
...
### Task 11.16: spec sentence **[NEEDS RULING]**, gates, PR, CI, exit-criteria evidence

**Files:**
- Modify: `docs/design/2026-07-05-gas-camp-design.md` §10 (one sentence, ONLY if the lead approves it at plan review)

- [ ] **Step 1: The proposed spec §10 addition** (after the stall mechanism paragraph): *"campd-spawned workers run in stream-json input mode with campd holding the stdin pipe (the live nudge path); when a worker's bead closes, campd releases it — stdin EOF, a bounded grace, then termination recorded as `session.stopped` with the reason — because an idle stream worker does not exit on its own (probe-verified, claude 2.1.204)."* If the ruling rejects Decision C, this step is void.
...
- [ ] **Step 3: Push + PR** — `git push -u origin phase-11-patrol-adoption`; `gh pr create` titled "Phase 11: health patrol and adoption" with: the probe findings summary (P1 resolves the carried PR #14 note), the Decision C/D rulings as approved, the phase-9 coordination note (capture format, settle signature), and the exit-criteria evidence table.
