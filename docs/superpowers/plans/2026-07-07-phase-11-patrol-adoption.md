...
## Execution amendments (2026-07-08, discovered while implementing — recorded for the code review)

1. **Targeted respawn (`Dispatcher::dispatch_bead`), Tasks 11.9/11.11/11.15.** The plan assumed a patrol-killed bead would respawn through the normal `converge` — but Phase 8's `dispatchable_beads` DELIBERATELY excludes ever-sessioned beads (decision C, pinned by `dispatchable_still_excludes_after_bound_session_ends`: organic crashes must not hot-loop before Phase 9's retry budgets). Patrol restarts are ladder-budget-bounded, so they re-hook the bead through an explicit `dispatch_bead(ledger, bead_id)` — queued as `PendingAction::Respawn` when patrol observes a `session.crashed` whose reason starts with `"patrol restart"`. A cap-full dispatcher queues the respawn and retries it when a slot frees (round-2 LOW 2 below — the original "record dispatch.failed and drop" was the finding). The general dispatchable set is untouched; the `readiness.rs` comment now names the seam.
2. **Canonical watch keys, Task 11.10.** Watch backends (FSEvents/inotify) report CANONICALIZED paths; the filter's exact-match on the raw registry path never hit under a symlinked prefix (macOS `/var` → `/private/var` — every tempdir; symlinked homes generally). `watch_transcript` canonicalizes the watched parent and keys the filter (and `path_to_session`) on `canonical_parent/file_name`; `Tracked.watch_key` carries it for unwatch. Caught by the end-to-end heartbeat integration test — the unit tests drove the callback directly and could not see it.
3. **Wake-anchored re-arm grace, Tasks 11.10/11.15.** `declare_stalls` gains a `now` parameter and re-arms at `max(fire.deadline, now)`: anchoring only at the fired deadline would let a lagging wake schedule the next fire in the past — escalating nudge→restart in one breath and giving a delivered nudge zero revival time. Deterministic in tests (pass the fire instant), grace-correct under lag.
4. **Hermetic git fixtures (all worktree tests).** The git fixtures now set `commit.gpgsign=false`: the suite must never depend on the host's commit-signing agent (a locked signing agent after a laptop sleep failed three merged-phase tests that inherited the global config).

## Round-2 review fixes (PR #20 Opus review, request_changes — all four fixed on this branch)

**HIGH 1 — stale-woke `Owned::Child` misclassification defeated restart AND release for adoption-window orphans.** A worker orphaned during the `converge`→`catch_up` window (multi-spawn worktree creation, seconds wide) leaves its `session.woke` row past the cursor; the restart's startup settle replays it and `observe_woke` classified it `Owned::Child` by `actor=="campd"` alone, before `adopt()` could probe it (and `adopt()` then skipped it as already-tracked). `do_restart`'s `Owned::Child` arm called `kill_worker()` and DISCARDED its bool — false for a non-child, so no kill, no `session.crashed`, no bead release, while a false `agent.stalled{restart}` burned ladder budget on fiction; the bead-close Release path failed identically (the fallback matched only `AdoptedPid`), so a finished orphan never got its `session.stopped` — the operator-adopted spec §10 release sentence violated for exactly these workers. **Fix (both halves the reviewer offered):** (1) `do_restart` trusts the LIVE dispatcher — only a `kill_worker` that RETURNS TRUE takes the SIGCHLD path; every other case (including a "Child" this campd does not hold) falls through to `restart_non_child`, the probe-verified non-child kill (extracted from the old `AdoptedPid` arm); the Release fallback now matches ANY tracked session `release_worker` declined that is not `Owned::Annotate` (never an attended session, spec §10). (2) `apply_tracking` skips a `Track` for an already-tracked session name (fold-unique → a duplicate is stale replay), keeping the first classification authoritative. Regression pin `an_orphan_retracked_as_child_is_still_restarted_and_released`: two live uuid-tagged orphans the fresh dispatcher does not hold, driven through Restart and BeadClosed MID-WINDOW — both processes actually die and the caused `session.crashed`/reasoned `session.stopped` land (the existing kill-9 test waits for both claims before killing, which is why it missed this).

**LOW 2 — cap-full patrol respawn stranded the bead with a false "deferred" event** (reachable from the `AdoptedPid` restart path, whose non-child kill frees no `children` slot). `dispatch_bead` now QUEUES the deferral in `pending_respawns`, events it ONCE with a truthful "will retry when a slot frees" reason, and `converge` drains the queue first on every wake — a reap-freed slot re-hooks the oldest deferral, never stranded (invariant 3). Phase 9's retry machinery will subsume budget/backoff (the hand-off seam is named in the comment). Pin `a_cap_full_patrol_respawn_queues_and_retries_when_a_slot_frees`.

**LOW 3 — `pgrep -f <uuid>` substring could SIGKILL an operator's inspection process** (a `tail -f <transcript>` carries the uuid). `probe_alive` now filters candidates through `pid_runs_command`, which accepts a pid only if its leading argv tokens' basename matches the configured `[dispatch]` command — biased toward under-matching (a miss degrades to a visible respawn, never a wrong kill). Pin `probe_ignores_processes_that_are_not_the_worker_command` (a decoy carrying the uuid but not the worker command is invisible). Side benefit: a just-killed zombie (empty/gone command line) reads as dead, making the probe-to-kill re-probe robust.

**LOW 4 — `kill_pid` classified the probe-to-kill race by locale-dependent stderr text** (`"No such process"`). `kill_pid` no longer consults the kill's exit chatter at all; every caller classifies the outcome by RE-PROBING (the `do_restart` path already did — extended to `release_adopted` and the `adopt()` non-child release). Pin `kill_pid_never_classifies_by_stderr_text` (pid 1 → EPERM is not an error).
