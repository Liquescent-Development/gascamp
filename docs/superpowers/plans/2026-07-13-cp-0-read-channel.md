...
> **Amendment review: AMENDMENT_APPROVED, 2026-07-13 (retrospective adversarial amendment-reviewer).** A retrospective adversarial audit (2026-07-12) found 10 blocking defects in the plan as originally approved; a fresh adversarial amendment-reviewer ruled AMENDMENT_APPROVED and the following 10 binding fixes were applied by the implementer. This supersedes the insufficient original single-reviewer approval below for the items it touches.
>
> 1. **PatrolDegraded fold-contract (embed in error string).** `take_watch_error_events` / `take_parse_error_events` / `take_drain_error_events` drop the `component`/`offset`/`line` keys — the `PatrolDegraded` fold struct is `deny_unknown_fields` (only `error` + `session`); the read-channel source id, offset, and offending line ride the `error` STRING. Added a camp-core Ledger fold-roundtrip unit test (`read_channel_patrol_degraded_shapes_round_trip_through_the_fold`) that appends each shape and asserts a clean refold.
> 2. **max_stream_bytes ceiling — real injectable-cap test.** `mod.rs` reads `CAMP_MAX_STREAM_BYTES` (u64, fall back to `MAX_STREAM_BYTES_DEFAULT`) via `read_channel::max_stream_bytes_from_env`. The Task 7 ceiling integration test (`max_stream_bytes_breach_fails_the_session_loudly`) spawns campd with `CAMP_MAX_STREAM_BYTES=64`, grows a session stdout file past it, and asserts in order: `session.stream_capped` (with `cap_bytes`+`session`) → `session.crashed` (with `cause_seq` == the stream_capped seq) → the bead re-hooks (a second `session.woke`). Added a `kill_worker_with_reason` + reap-arm unit test in `dispatch.rs` (`cap_breach_kill_worker_with_reason_reaps_as_crashed_with_the_named_reason`).
> 3. **Task 2 fold-arm ordering (doc correction).** The `EventType::SessionStreamCapped => Ok(())` fold arm was added ALONGSIDE the variant (Step 1) so Step 2's vocab_pin red is a real test failure, not E0004. No code change — recording the doc correction.
> 4. **Task 6 Step 5 reap snippet — drop the semicolon.** The reap arm is a tail expression `EventType::SessionCrashed` (no semicolon), matching the existing `dispatch.rs:864-867` shape.
> 5. **Attended-session filter (phase-0 blocking).** `observe()` gates the `SessionWoke` Register on `event.actor == "campd"`; the `mod.rs` seeding loop filters `row.woke_actor == "campd"`. Added a unit test (`observe_filters_attended_sessions_and_registers_only_campd_workers`): a `SessionWoke` with `actor: "hook:session-start"` queues NO Register; a campd-actor `SessionWoke` does.
> 6. **Cap-kill re-hook (fix = a, prefix the reason).** The kill reason is `"patrol restart: stream cap exceeded max_stream_bytes"` so `patrol::observe` (patrol.rs:301-304) matches the `patrol restart` prefix → queues Respawn → the bead re-hooks (the ceiling integration test verifies the second `session.woke`).
> 7. **Offset-persist AFTER event commit (phase-0 blocking).** Deleted the in-loop `set_stream_cursor` from `drain_one` (in-memory offset only); added `pub fn persist_offsets(&mut self, ledger)` that persists every tailed session's in-memory offset; the event-loop drain block calls it as the LAST step (after `take_parse_error_events` AND `take_cap_breaches`). Task 4 unit tests that assert the persisted cursor now call `persist_offsets` after `drain_all`.
> 8. **Fatal drain `?` → non-fatal (phase-0 blocking).** `drain_one` captures per-session open/seek/read/stat errors into a `drain_errors` collector + `eprintln!` + returns `Ok(())` (a missing file still skips); `drain_all` is non-fatal in the event loop (`if let Err(e) = drain_all { eprintln }`). Added `take_drain_error_events` (PatrolDegraded, fix-1 shape) appended alongside the parse-error events. `apply_tracking(ledger)?` stays fatal (no file I/O).
> 9. **OOM-before-cap (phase-0 blocking).** `drain_one` adds a PRE-read cap check (file_size > cap → CapBreach, no read) AND an IN-LOOP before-extend check (`partial.len() + n > cap` → CapBreach, break without extending, so `partial` never exceeds the cap). Added a unit test (`drain_all_surfaces_a_cap_breach_for_a_huge_newlineless_line_without_unbounded_partial`): a newline-less line larger than the cap breaches and `partial` stays ≤ cap (RSS-bounded).
> 10. **Reap-time stream-file disposal (phase-0 blocking).** `unregister` best-effort `std::fs::remove_file`s the session's stdout file after clearing the cursor (§2.3: "stream files append-only until reap"). Added a unit test (`unregister_disposes_the_session_stream_file`); updated `daemon_dispatch.rs`'s Tier-0 assertion to verify disposal (the file is gone after the session stops) instead of post-stop existence.
>
> *Recorded by the implementer session per the two-session split; the 10 fixes are committed alongside this note.*

> **Plan review: APPROVE, 2026-07-13 (plan-gate review).** Plan execution-ready — covers all §2.3 read-channel obligations, the §4.3 perf-gate extension (N subscribers correctly deferred to phase 2 per §7/§4.4 — the phase-0 socket is one-shot), and all three phase-0 §8 state-machine tests (read-on-wake, Rescan drain, append-only cursors across a campd restart). Four flagged items ruled: **(a)** the additive `dispatch.rs` change (`kill_worker_with_reason` + `kill_reason` field + reap-classification line at dispatch.rs:844-871, the SIGCHLD reap path) — conditionally justified (additive to current main; structurally disjoint from fix-83's failed-set/ready-scan/adopt-retry region); **LEAD PERMITS** the `dispatch.rs` touch with the standard rebase-on-fix-83-merge protocol. **(b)** `max_stream_bytes` as a module constant `MAX_STREAM_BYTES_DEFAULT` (256 MiB) — justified; operator-configurability deferred (§2.3 "(config, generous default)" is the end-state design, not a phase-0 gate). **(c)** N subscribers deferred to phase 2 — justified (`session.subscribe` is a phase-2 verb; phase-0 has no long-lived subscription connection). **(d)** `stream_cursors` via idempotent `CREATE TABLE IF NOT EXISTS` without a `SCHEMA_VERSION` bump — conditionally justified (consumer bookkeeping, not fold state; inert table; loss = re-read from offset 0, never corruption); **LEAD ACCEPTS** the no-bump approach, REQUIRING a code comment documenting the consumer-bookkeeping evolution path (idempotent `CREATE TABLE IF NOT EXISTS`, no version bump) vs. the fold-state version-gated path (`FULL_DDL_PREFIX` + `SCHEMA_VERSION`). **Non-blocking notes (address during execution):** (1) make `max_stream_bytes` test-injectable (test-only env var or test-only constructor path the `mod.rs` wiring reads) so a real §8 ceiling integration test with a small cap exercises the full path (`session.stream_capped` → `kill_worker_with_reason` → reap `session.crashed` with `cause_seq` → bead re-hook), not a placeholder; (2) Task 6 Step 5 reap snippet — drop the errant trailing semicolon on `EventType::SessionCrashed;` so the branch evaluates to the event variant; (3) Task 2 TDD ordering — add the `fold.rs` arm ALONGSIDE the `SessionStreamCapped` variant (not in a later step) so the crate compiles at each red/green step (otherwise Step 2's "run the vocab test to verify it fails" hits a compile error, not a test failure); then observe the vocab-partition test fail red, add the vocab entry to turn it green; (4) add a code comment at the offset-persistence site noting the phase-1+ obligation to reorder persist-after-event-commit when lines become `permission.pending` events; (5) use `spawn::munge` (public re-export if needed) in the integration-test scaffold so the stdout path matches what the runtime registers; (6) add a one-line comment at EACH `apply_tracking` call site stating which case it covers (the double call is a deliberate safety net for the no-settle-ran case, idempotent via `mem::take`-drained `track_ops`); (7) reusing `PatrolDegraded` with `component: "read_channel"` for watcher errors / non-JSON lines is acceptable for phase 0; split a dedicated `read_channel.degraded` later if the component field becomes noisy. *Recorded by the implementer session per the two-session split.*
