...
## Execution notes (deviations found while executing, 2026-07-09)

1. **The design's assumed fail-fast mechanism does not hold on modern git — an explicit base-commit check was added.** Design §4.2.2 assumed "`git worktree add -b camp/<bead>` requires a git repo with a base commit; on a freshly `git init`'d rig with zero commits it fails." Verified false on git 2.55 (this machine and current runners): git ≥2.42 auto-infers `--orphan` and SUCCEEDS on an unborn HEAD (observed: `No possible source branch, inferring '--orphan'`), silently creating a baseless worktree — exactly the stranded-work hazard the contract forbids, caught red-handed by obligation-(ii)'s test. Fix: `ensure_worktree_base` in `crates/camp/src/daemon/spawn.rs` — `git -C <rig> rev-parse --verify HEAD^{commit}` before any side effect; failure bails "rig … cannot host a worktree (no git repository with a base commit): <git stderr>", which lands as `dispatch.failed` through the existing launch path. This is the design-blessed mechanical check (design §2, invariant 4: "a mechanical 'is this a git repo with a base commit' check"), covers both obligation-(ii) shapes (git-init-only AND not-a-repo) through one rev-parse, and makes the contract deterministic across git versions. Unit test: `create_worktree_refuses_a_rig_without_a_base_commit` (spawn.rs). The integration tests assert the reason contains "cannot host a worktree" (not "git worktree add failed" as originally planned).
2. **Rebased onto main after PR #49 (issue-44 de-flake) merged.** `FAKE_AGENT_RECORD_BRANCH` was reconciled with #49's ordering contract: all proof files (TOUCH, RECORD_BRANCH) are written BEFORE the claim, so a ledger-observed claim implies they exist. Obligation-(i)/(iii) tests use direct asserts after the claim wait (main's de-flaked idiom) instead of the planned poll loops.
3. **One extra re-pin beyond the plan's list:** the dispatch.rs unit-test fixture in `a_cap_full_patrol_respawn_queues_and_retries_when_a_slot_frees` (plain-dir rig; subject is the respawn queue) pins `isolation: none`.
4. Task 3's `.clone()` hedge was not needed — `serde_json::json!` serialized `prep.agent_name` by reference, as the plan reviewer predicted.
