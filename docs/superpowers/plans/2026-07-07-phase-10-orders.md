...
### Fix-pass round 2 (2026-07-07, PR #13 fix-pass review)

9. **NEW MEDIUM (amends amendment 2 / Decision F):** the cursor-position anchor alone could not see campd's own `order.fired` events appended but unprocessed at death — a kill -9 between a declaration and its settle let the restart's recompute re-declare the same (order, scheduled) fire after settle cooked the original: a double cook. Cron-fire declaration is now idempotent: `declare_cron_fires` (the single declaration path for the loop, jump, and startup sites) probes `orders::cron_fire_declared` — an indexed (order, scheduled_ts) existence check via `Ledger::has_event_with_data_strs` — before appending; re-declaring an already-declared occurrence is never legitimate. Test: `a_fire_crashed_before_its_settle_cooks_exactly_once_on_restart` (proven red pre-fix: 2 declarations, 2 cooks).
10. **LOW (amends Decision G's flag semantics):** the jump path stamped every recomputed fire `catch_up: true` while `fire_due` computed lateness. The flag now has one home — `CatchUp::into_fire(now)` (lateness > `ON_TIME_TOLERANCE`) — used by the loop's jump branch and the startup recompute. Test: `into_fire_computes_the_catch_up_flag_from_lateness` (asserts flag equality with `fire_due` for the identical wake state).
11. **LOW:** Decision H's token sentence carried the pre-ruling layout; it now points at amendment 4 (Token(2) reserved for Phase 8's SIGCHLD; connections from 3).
